...
class PrefectsSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $villagers = json_decode(file_get_contents(realpath(__DIR__ . '/bakhshdars.json')), true);

        DB::transaction(function () use ($villagers) {

            foreach ($villagers as $villager) {
                $names=explode('-',$villager['username']);
                $villager['firstName']=$names[0];
                $villager['lastName']=$names[1];
                $villager['nationalCode']=rand(100000000,9999999999);
                    ...
                    $villager['password'] = (string)$villager['mobile'];
...
//                        dd($villager);
//                    }
                $districtOU = $district->organizationUnit;

                $districtOU->head_id = $userResult->id;
                $districtOU->save();
