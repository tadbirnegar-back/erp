...
class LoginControllerV2 extends Controller
{
    ...
    public function loginGrant(Request $request)
    {
        ...
        $person = $user->load(['person' => function ($query) {
        ...
        $isDehyar = in_array('کاربر', $roles->toArray());

        $result = [
            'sidebar' => $permissions ?? null,
            'userInfo' => $userInfo ?? null,
            'roles' => $roles ?? null,
            'hasPayed' => $isDehyar ? $checkUsersPayment['hasDebt'] : true,
            'confirmed' => $isDehyar ? $this->userVerified($user) : true,
            'version' => $versions
        ];

        if($isDehyar){
            $result['phaseOnePay'] = $checkUsersPayment['alreadyPayed'];
        }
